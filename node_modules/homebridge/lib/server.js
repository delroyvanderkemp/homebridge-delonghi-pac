...
class Server {
    constructor(options = {}) {
        ...
        this.bridge = new _1.Bridge(this.config.bridge.name, _1.uuid.generate("HomeBridge"));
    }
    async start() {
        const promises = [];
    ...
    publishBridge() {
        ...
        const info = this.bridge.getService(_1.Service.AccessoryInformation);
        info.setCharacteristic(_1.Characteristic.Manufacturer, bridgeConfig.manufacturer || "homebridge.io");
        info.setCharacteristic(_1.Characteristic.Model, bridgeConfig.model || "homebridge");
        info.setCharacteristic(_1.Characteristic.SerialNumber, bridgeConfig.username);
        info.setCharacteristic(_1.Characteristic.FirmwareRevision, version_1.default());
        ...
        const publishInfo = {
            ...
            category: _1.Accessory.Categories.BRIDGE,
    ...
    restoreCachedPlatformAccessories() {
        this.cachedPlatformAccessories = this.cachedPlatformAccessories.filter(accessory => {
            ...
            if (!platformPlugins) {
                ...
                accessory.getService(_1.Service.AccessoryInformation)
                    .setCharacteristic(_1.Characteristic.FirmwareRevision, plugin.version);
    ...
    async loadPlatformAccessories(plugin, platformInstance, platformType) {
        // Plugin 1.0, load accessories
        return new Promise(resolve => {
            platformInstance.accessories(_1.once((accessories) => {
    ...
    createHAPAccessory(plugin, accessoryInstance, displayName, accessoryType, uuidBase) {
        ...
        if (!(services[0] instanceof _1.Service)) {
            ...
            return _1.AccessoryLoader.parseAccessoryJSON({
            ...
            const accessoryUUID = _1.uuid.generate(accessoryType + ":" + (uuidBase || displayName));
            const accessory = new _1.Accessory(displayName, accessoryUUID);
            ...
            services.forEach(service => {
                ...
                if (service instanceof _1.Service.AccessoryInformation) {
                    const existingService = accessory.getService(_1.Service.AccessoryInformation);
                    service.setCharacteristic(_1.Characteristic.Name, displayName); // ensure display name is set
    ...
    handleRegisterPlatformAccessories(accessories) {
        const hapAccessories = accessories.map(accessory => {
            ...
            const plugin = this.pluginManager.getPlugin(accessory._associatedPlugin);
            if (plugin) {
                const informationService = accessory.getService(_1.Service.AccessoryInformation);
                if (!informationService.testCharacteristic(_1.Characteristic.FirmwareRevision)) {
                    informationService.setCharacteristic(_1.Characteristic.FirmwareRevision, plugin.version);
                }
    ...
    handlePublishExternalAccessories(accessories) {
        ...
        accessories.forEach(accessory => {
            ...
            const plugin = this.pluginManager.getPlugin(accessory._associatedPlugin);
            if (plugin) {
                const informationService = hapAccessory.getService(_1.Service.AccessoryInformation);
                if (!informationService.testCharacteristic(_1.Characteristic.FirmwareRevision)) {
                    informationService.setCharacteristic(_1.Characteristic.FirmwareRevision, plugin.version);
                }
            }
