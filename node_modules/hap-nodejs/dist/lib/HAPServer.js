...
var HAPServer = /** @class */ (function (_super) {
    ...
    function HAPServer(accessoryInfo, relayServer) {
        ...
        _this._onRemoteRequest = function (request, remoteSession, session, events) {
            ...
            if (request.messageType === "pair-verify" /* PAIR_VERIFY */)
                _this._handleRemotePairVerify(request, remoteSession, session);
            else if (request.messageType === "discovery" /* DISCOVERY */)
                _this._handleRemoteAccessories(request, remoteSession, session);
            else if (request.messageType === "write-characteristics" /* WRITE_CHARACTERISTICS */)
                _this._handleRemoteCharacteristicsWrite(request, remoteSession, session, events);
            else if (request.messageType === "read-characteristics" /* READ_CHARACTERISTICS */)
        ...
        _this._handlePairStepFive = function (request, response, session, clientUsername, clientLTPK, hkdfEncKey) {
            ...
            _this.emit("pair" /* PAIR */, clientUsername.toString(), clientLTPK, once_1.once(function (err) {
                ...
                response.writeHead(200, { "Content-Type": "application/pairing+tlv8" });
                response.end(tlv.encode(6 /* SEQUENCE_NUM */, 0x06, 5 /* ENCRYPTED_DATA */, Buffer.concat([ciphertextBuffer, macBuffer])));
                session._pairSetupState = undefined;
            }));
        };
        /**
        ...
        _this._handlePairVerifyStepOne = function (request, response, session, objects) {
            ...
            response.writeHead(200, { "Content-Type": "application/pairing+tlv8" });
            response.end(tlv.encode(6 /* SEQUENCE_NUM */, 2 /* M2 */, 5 /* ENCRYPTED_DATA */, Buffer.concat([ciphertextBuffer, macBuffer]), 3 /* PUBLIC_KEY */, publicKey));
        ...
        _this._handleRemotePairVerifyStepOne = function (request, remoteSession, session, objects) {
            ...
            var macBuffer = Buffer.alloc(16);
            encryption.encryptAndSeal(outputKey, Buffer.from("PV-Msg02"), message, null, ciphertextBuffer, macBuffer);
            var response = tlv.encode(6 /* SEQUENCE_NUM */, 0x02, 5 /* ENCRYPTED_DATA */, Buffer.concat([ciphertextBuffer, macBuffer]), 3 /* PUBLIC_KEY */, publicKey);
        ...
        _this._handleRemotePairVerifyStepTwo = function (request, remoteSession, session, objects) {
            ...
            if (!encryption.verifyAndDecrypt(enc.hkdfPairEncKey, Buffer.from("PV-Msg03"), messageData, authTagData, null, plaintextBuffer)) {
                debug("[%s] M3: Invalid signature", _this.accessoryInfo.username);
                var response = tlv.encode(6 /* STATE */, 4 /* M4 */, 7 /* ERROR_CODE */, 2 /* AUTHENTICATION */);
                remoteSession.responseMessage(request, response);
                return;
            }
            var decoded = tlv.decode(plaintextBuffer);
            ...
            enc.accessoryToControllerKey = hkdf.HKDF("sha512", encSalt, enc.sharedSec, infoRead, 32);
            enc.controllerToAccessoryKey = hkdf.HKDF("sha512", encSalt, enc.sharedSec, infoWrite, 32);
            var response = tlv.encode(6 /* SEQUENCE_NUM */, 0x04);
            remoteSession.responseMessage(request, response);
            session.establishSession(clientUsername.toString());
        };
        /**
        ...
        _this._handleRemoteCharacteristicsWrite = function (request, remoteSession, session, events) {
            ...
            // call out to listeners to retrieve the latest accessories JSON
            _this.emit("set-characteristics" /* SET_CHARACTERISTICS */, data, events, once_1.once(function (err, characteristics) {
                if (err) {
                    debug("[%s] Error setting characteristics: %s", _this.accessoryInfo.username, err.message);
                    // rewrite characteristics array to include error status for each characteristic requested
                    characteristics = [];
                    for (var i in data) {
                        characteristics.push({
                            aid: data[i].aid,
                            iid: data[i].iid,
                            s: -70402 /* SERVICE_COMMUNICATION_FAILURE */
                        });
                    }
                }
        ...
        _this._handleRemoteCharacteristicsRead = function (request, remoteSession, session, events) {
            var data = JSON.parse(request.requestBody.toString());
            _this.emit("get-characteristics" /* GET_CHARACTERISTICS */, data, events, function (err, characteristics) {
                if (!characteristics && !err)
                    err = new Error("characteristics not supplied by the get-characteristics event callback");
                if (err) {
                    debug("[%s] Error getting characteristics: %s", _this.accessoryInfo.username, err.stack);
